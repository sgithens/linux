...
// Handle<Value> add_gsetting_schema(const Arguments& args) {
Handle<Value> add_gsetting_schema(const Arguments& args) {
  HandleScope scope;
  GSettings *settings;
  gchar ** keys;
  ...
  char directory[1024];
  char path[1024];
  args[0]->ToString()->WriteAscii(directory);
  fprintf(stderr, "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK\n%s", directory);
  fprintf(stderr, "Reading schema from directory: %s\n", directory);
  ...
  schemaSource = g_settings_schema_source_new_from_directory(directory, NULL, FALSE, NULL);
...
void init(Handle<Object> target) {
  ...
  target->Set(String::NewSymbol("add_gsetting_schema"),
              FunctionTemplate::New(add_gsetting_schema)->GetFunction());
}

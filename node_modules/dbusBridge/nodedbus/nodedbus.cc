  ...
  if (args[4]->IsBoolean()) {
    ...
    for(i=0;i<array->Length();i++){
      ...
      if (array->Get(i)->IsBoolean()) {
        gboolean bool_value;
        bool_value = array->Get(i)->BooleanValue();
        g_value = g_variant_new("b", bool_value);
        printf("Value %d - Boolean - %d\n", i, bool_value);
      }
      else if (array->Get(i)->IsNumber()) {
        g_value = g_variant_new("i", array->Get(i)->NumberValue());
        printf("Value %d - Number - %d\n", i, array->Get(i)->NumberValue());
      }
      else if (array->Get(i)->IsString()) {
        char str_value[1024];
        array->Get(i)->ToString()->WriteUtf8(str_value);
        g_value = g_variant_new("s", str_value);
